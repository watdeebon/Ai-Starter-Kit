...
(function() {
    ...
    const CONFIG = {
        ...
        USE_REMOTE_QUESTIONS: false, // เปลี่ยนเป็น true เพื่อใช้คำถามจาก Google Sheets
        ENABLE_SOUND: true, // เปิด/ปิดเสียง
        ENABLE_VIBRATION: true, // เปิด/ปิดการสั่น
        STREAK_BONUS: true, // โบนัสตอบถูกติดต่อกัน
    ...
    let gameState = {
        ...
        leaderboard: [],
        gameStarted: false,
        questions: [], // เพิ่มเพื่อเก็บคำถามที่โหลดมา
        streak: 0, // จำนวนข้อที่ตอบถูกติดต่อกัน
        powerUps: {
            fiftyFifty: 1, // ลบตัวเลือกผิด 2 ตัว
            extraTime: 1,  // เพิ่มเวลา 15 วินาที
            skipQuestion: 1 // ข้ามข้อ
        },
        usedPowerUps: [], // Power-ups ที่ใช้แล้วในข้อปัจจุบัน
        achievements: [], // ความสำเร็จที่ได้รับ
        totalScore: 0, // คะแนนรวมทั้งหมด
        questionStats: [] // สถิติการตอบแต่ละข้อ
    };

    // Achievements system
    const ACHIEVEMENTS = {
        FIRST_PLAY: { name: '🎮 เล่นครั้งแรก', desc: 'เล่นเกมครั้งแรก', points: 5 },
        PERFECT_SCORE: { name: '💯 คะแนนเต็ม', desc: 'ได้คะแนนเต็ม 100+', points: 20 },
        SPEED_DEMON: { name: '⚡ ปีศาจความเร็ว', desc: 'ตอบทุกข้อภายใน 10 วินาที', points: 15 },
        STREAK_MASTER: { name: '🔥 ต่อเนื่อง', desc: 'ตอบถูก 5 ข้อติดต่อกัน', points: 10 },
        NO_POWERUP: { name: '🏆 ไม่ใช้ความช่วยเหลือ', desc: 'จบเกมโดยไม่ใช้ Power-up', points: 15 },
        COMEBACK_KING: { name: '👑 กษัตริย์แห่งการกลับมา', desc: 'ตอบผิด 3 ข้อแรกแต่จบด้วยคะแนนดี', points: 12 }
    };

    // Sound effects (ถ้าต้องการเพิ่มเสียง)
    const SOUNDS = {
        correct: () => { if (CONFIG.ENABLE_SOUND) playBeep(800, 200); },
        incorrect: () => { if (CONFIG.ENABLE_SOUND) playBeep(400, 300); },
        tick: () => { if (CONFIG.ENABLE_SOUND) playBeep(600, 100); },
        finish: () => { if (CONFIG.ENABLE_SOUND) playMelody(); }
    };
    ...
    function vibrate(pattern = 200) {
        if (CONFIG.ENABLE_VIBRATION && navigator.vibrate) {
